...
export const metadata: Metadata = {
  title: "Fisiatría UCN — Repositorio",
  description: "Repositorio de Medicina Física y Rehabilitación de la Universidad Católica del Norte.",
};
  ...
  if (!featured) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-center">
        ...
        <h2 className="font-headline text-2xl font-bold text-on-surface mb-2">Sin publicaciones aún</h2>
        <p className="text-on-surface-variant">El repositorio está vacío. Ejecuta el script de seed para poblar los datos.</p>
      </div>
    );
  }
  ...
  return (
    <>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <span className="font-label text-sm uppercase tracking-widest text-secondary font-bold mb-2 block">
              INVESTIGACIÓN DE ELITE
            </span>
            <h1 className="font-headline text-4xl md:text-5xl font-extrabold text-on-surface tracking-tight max-w-2xl leading-tight">
              Repositorio de Medicina Física y Rehabilitación.
            </h1>
          </div>
          <div className="flex flex-wrap gap-2">
            <Badge variant="outlined-primary">AJPM&amp;R</Badge>
            <Badge variant="outlined-secondary">Minerva Medica</Badge>
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            ...
            <div className="p-8 md:w-3/5 flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-3 mb-4">
                  <Badge variant="filled-primary" size="sm">DESTACADO {featured.journal}</Badge>
                  <span className="text-gray-500 text-xs font-medium">{featured.publishedAtLabel}</span>
                </div>
                <h2 className="font-headline text-2xl font-bold text-gray-900 mb-4 leading-snug group-hover:text-primary transition-colors">
                  {featured.title}
                </h2>
          ...
          <article className="md:col-span-4 bg-white border border-gray-100 rounded-xl editorial-shadow p-6 flex flex-col justify-between hover:border-secondary transition-all cursor-pointer">
            <Link href={card2.href} className="flex flex-col h-full justify-between">
              <div>
                ...
                <h3 className="font-headline text-lg font-bold text-gray-900 mb-3">{card2.title}</h3>
                <p className="text-gray-600 text-sm leading-relaxed line-clamp-4">{card2.excerpt}</p>
              </div>
              <div className="mt-6 flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-secondary">Guardar en Biblioteca UCN</span>
              </div>
            </Link>
          </article>
        )}
        ...
        {card3 && (
          <article className="md:col-span-4 bg-gray-50 rounded-xl p-6 flex flex-col border-l-4 border-primary hover:shadow-md transition-all cursor-pointer">
            <Link href={card3.href} className="flex flex-col h-full">
              ...
              <h3 className="font-headline text-lg font-bold text-gray-900 mb-3">{card3.title}</h3>
              <p className="text-gray-600 text-sm leading-relaxed line-clamp-3 mb-4">{card3.excerpt}</p>
              <div className="mt-auto pt-4 flex justify-between items-center">
                <span className="text-xs font-bold text-primary">{card3.journal}</span>
                <span className="text-[10px] text-gray-500">{card3.publishedAtShort}</span>
              </div>
            </Link>
          </article>
        )}
        ...
        {card4 && (
          <article className="md:col-span-4 bg-white border border-gray-100 rounded-xl editorial-shadow p-6 group cursor-pointer">
            <Link href={card4.href} className="flex flex-col h-full">
              ...
              <h3 className="font-headline text-lg font-bold text-gray-900 mb-3 leading-tight">{card4.title}</h3>
              <p className="text-gray-600 text-sm leading-relaxed line-clamp-3">{card4.excerpt}</p>
        ...
        <StatsCard
          ...
          description="El factor de impacto de nuestras fuentes AJPM&R y Minerva Medica lidera el sector este año."
          stats={[
      ...
      <section className="mt-20">
        <SectionHeader
          title="Resúmenes de Archivo"
          action={{ label: "Histórico", href: "/buscar", icon: "history" }}
        />
